...
class ImageSave(BaseModel):
    ...
    table_name: str = Field(
        description="Название таблицы, к объекту которой относится картинка"
    )
    image_name: str | None = Field(
        default=None, description="название используется для атрибута alt тега img"
    )
...
class ImageUpdate(BaseModel):
    ...
    image_type: str = Field(
        description="тип картики. Ожидается image_desktop, или image_mobile"
    )
    image_action: str = Field(
        description="тип действия над картикой. Ожидается image_delete, или image_refresh"
    )
